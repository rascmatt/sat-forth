...
: decide { dl a_addr -- i1 .. in in+1 dl b }
  ...
  a_addr @                      ( a_n )
  a_addr swap 2 * 2 + cells +   { h_addr } \ Calculate the offset of the decision score table
...
: initialize-decision-score { a_addr l_addr -- }
  ...
  \ > The current implementation initializes all scores with 0.
...
: decay-decision-score { a_addr -- }
  ...
  \ > The current implementation decays all scores by 5%
...
: resolve-conflict { dl a_addr l_addr -- .. dl }
  ...
  begin               ( c_addr )    
    ...
    is-asserting 0= while ( [ li dl cl ] c_addr )
      ...
      over -1 = IF   \ Backtracked to DL -1: Unsat
        r> drop
        is_new free throw
        -1 exit
      THEN
...
: is-sat { a_addr l_addr -- b }
  ...
  begin
    ...
    \ TODO: diable fixed restarts -> termination issues for unsat formulas
    iteration @ 200 mod 0= IF
      a_addr -1 backtrack
      drop 0
      a_addr l_addr initialize-decision-score
    THEN
    ...
    0= IF
      ...
      \ Decay after every 100th conflict
      ...
      \ Dump the decision scores
      \ a_addr print-decision-score
